// app/attendance/teacher/page.js
// The teacher's attendance screen.
//
// ONE PAGE, TWO VIEWS, decided by the URL (no client-side screen switching):
//   /attendance/teacher            -> the class grid (ClassPicker)
//   /attendance/teacher?classId=49 -> the marking sheet for that class
//
...
export default async function TeacherAttendancePage({ searchParams }) {
	...
	if (!rawClassId) {
		...
		return (
			<div className="mx-auto w-full max-w-2xl px-4 pt-6 pb-12">
				...
				<ClassPicker classes={classes} basePath="/attendance/teacher" />
			</div>
		);
	}
	...
	return (
		<div className="mx-auto w-full max-w-2xl px-4 pt-6 pb-12">
			<BackLink href="/attendance/teacher" label="CLASSES" />
