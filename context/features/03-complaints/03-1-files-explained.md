# Feature 03 — Complaints: Every File Explained in Plain English

**Built:** 2026-08-14 · **Status:** tested and working
(Feedback forms were deliberately postponed — see `03-2-feedback-future-spec.md`.)
...
Complaints is a two-sided help desk:

- A **parent** opens `/parent/complaints`, writes a subject and a description
  (text only — no photos, no emoji picker), and submits. Their past
  complaints are listed underneath with status chips: **Pending** (the office
  hasn't opened it yet), **Read**, **Resolved** — plus the school's reply
  when there is one.
- The **admins** (the office) immediately get a bell notification AND a phone
  buzz: "New complaint received". Tapping it opens `/admin/complaints`.
- The admin inbox looks like an email client: a ticket queue on the left
  ...
  phone, their children with classes), write a reply — optionally polishing
  rough notes with the **AI Draft** button — and finally **Mark Resolved**.
...
### A. `lib/repos/complaintRepo.js` — the database layer

Every database question about complaints lives here (the project rule: SQL —
the database language — exists only inside `lib/repos/`).
...
| `markRead(id, branchId)` | Opening a ticket flips unread → read. Already-read/resolved: no-op. |
| `toggleFlag(id, branchId)` | Flips the flag on/off and returns the new value. |
| `replyToComplaint(...)` | Stores the reply + who + when. **Never resolves.** An unread ticket being answered becomes "read". Note: there is ONE reply column — the latest reply replaces the previous one. |
...
### B. `lib/complaintConstants.js` — the shared limits
...
### C. `lib/ai.js` — the AI helper (server-only)

`draftReply(roughNotes, {subject, description})` — sends the admin's notes to
...
OpenAI-compatible service works — including a free self-hosted one (Ollama).
...
the app, and it NEVER sends anything by itself — it only fills the reply box.
...
| Route | Who | Job |
|---|---|---|
| `route.js` GET | parent = own list · admin = the queue | One URL, split by the role in the signed login cookie. |
| `route.js` POST | parent | File a complaint → then alert every admin (bell + buzz). The complaint is saved FIRST; an alert failure can never lose it. |
| `[id]/route.js` GET | admin | One ticket + the parent's children (for the profile popover). |
| `[id]/route.js` PATCH | admin | `action`: `read` / `flag` / `reply` / `resolve` — with the owner's lifecycle rules enforced on the server. Reply pings the parent (bell + buzz). |
| `copilot/route.js` POST | admin | `{notes, complaintId?}` → a draft reply, or a polite 503 when AI isn't configured. Writes nothing. |

### E. The components and pages

- **`components/complaints/ComplaintForm.js`** (browser) — the parent's form.
  Clears itself on success and refreshes the page's list so the new complaint
  appears on top instantly.
- **`app/parent/complaints/page.js`** (server) — the form + "My Past
  Complaints" with status chips and the school's reply in a green-edged box.
- **`components/complaints/TicketQueue.js`** (browser) — the admin inbox: the
  queue with flags and filter, the detail pane with the profile popover, the
  reply box with Send / AI Draft / Mark Resolved (which stays disabled — and
  says why — until a reply exists). On phones it shows list OR ticket with a
  back button; on desktop they're side by side.
- **`app/admin/complaints/page.js`** (server) — a thin shell: guard + title +
  the inbox component.

### F. Changes to EXISTING files (small, additive)

- `lib/repos/authRepo.js` — ONE new function, `listAdminIdsByBranch()`
  (the office's address book for the new-complaint alert).
- `app/admin/page.js`, `app/parent/page.js` — one COMPLAINTS link card each.
- `components/fees/ReceiptCard.js` — a "Download as PDF file" link (fees
  retrofit, below).
- `next.config.mjs` — `serverExternalPackages: ["pdfkit"]` so the PDF library
  can find its font files (removing this line breaks PDF downloads with a
  500; the reason is written in a comment there).
- `package.json` — `pdfkit` added (owner-approved).
...
from feature 04 stays as the on-screen view — nothing about it changed):

- **New route:** `app/api/fees/receipt/[receiptNumber]/route.js` — builds the
...
- **Known limitation (documented in code):** the PDF writes "Rs 25,500.00"
  because pdfkit's built-in font cannot draw the ₹ symbol. Screens keep ₹.
- The step-by-step pdfkit note that feature 04's docs pre-wrote ("section 5")
  was followed almost exactly — the one surprise (bundled font files 500ing)
...
1. **PDF download answers 500 with a font path error** → somebody removed
   `serverExternalPackages: ["pdfkit"]` from `next.config.mjs`, or the server
   wasn't restarted after a config change.
2. **AI Draft says "unavailable: missing from .env.local"** → that is the
...
3. **A brand-new page 404s** → the dev server cached its page list before the
   file existed: stop it, delete the `.next` cache folder, restart.
4. **A parent reports they didn't get the reply alert** → the reply itself is
