# Feature 03 (second half) — FEEDBACK FORMS: Build-Ready Specification

**Status: NOT BUILT (deliberately deferred by the owner, 2026-08-14).**
Complaints (the first half) is done — see `03-0-decisions.md` and
...
The office builds a questionnaire ("form template") once — e.g. a Parent
Satisfaction Survey — then **pushes** it to parents as a **campaign**. Parents
open the app, see a pending form, fill it in (text answers, single-choice
questions, 1–5 star ratings), and submit — once each; the form then disappears
...
## 2. The database (already exists — no migrations needed or allowed)

Frozen in `db/schema.sql`; seeded in `db/seed.sql`:

- `feedback_templates(id, branch_id, name, fields JSONB, created_at)` — a
  reusable form definition. `fields` = the question list.
- `feedback_campaigns(id, branch_id, template_id, title,
  status 'open'|'closed', created_at)` — one row per push.
- `feedback_responses(id, campaign_id, parent_id, answers JSONB, created_at,
  UNIQUE(campaign_id, parent_id))` — the UNIQUE constraint is what makes a
...
`students.parent_profile_id` when needed — reuse
`attendanceRepo.getChildrenOfParent`).

### 2.1 THE FIELD JSON SHAPE — CRITICAL, READ TWICE
...
- `type` ∈ `text` | `single_choice` | `rating` (`single_choice` adds
  `options: ["A","B",...]`; `rating` adds `max: 5`).
- `answers` in responses is `{ "q1": 4, "q2": 2, "q3": "Overall good." }` —
  keyed by field `id`.
...
- 40 seeded responses (parents where n % 10 = 0). **Test parent
  `9810000002` has NOT answered** — perfect for the fill-flow test.
...
responses — those join through `feedback_campaigns`).

## 4. The lifecycle

- Template: created once, reused forever. (Prompt defines no edit/delete —
  keep it that way in v1.)
- Campaign: `open` the moment it's pushed → admin closes it manually
...
- `lib/repos/feedbackRepo.js`:
  ...
  - `listCampaigns(branchId)` — each with response count
  - `getOpenCampaignsForParent(parentId, branchId)` — a LIST (a parent could
    have more than one pending; the prompt said "the open campaign" but a list
    is safer)
  - `getCampaignForFill(campaignId, branchId)` — campaign + template fields
  - `submitResponse({campaignId, parentId, answers})` — rely on the UNIQUE
    constraint; return a 409-shaped outcome on duplicate; also refuse when the
    campaign is closed or another branch's
  - `getPendingParents(campaignId, branchId)` — branch parents (role='parent',
    via their children's branch... decide: all `profiles` with role 'parent'
    in the branch) minus responders
  - `getCampaignResults(campaignId, branchId)` — all responses + template

API routes (`app/api/feedback/`):
- `templates/route.js` — GET list, POST create (admin)
- `campaigns/route.js` — GET list (admin) | GET `?open=1` pending-for-me
  (parent) | POST push (admin) | PATCH close (admin)
- `respond/route.js` — POST (parent; branch + open + not-already-answered)
- `report/[campaignId]/route.js` — GET (admin): the PDF report
- `campaigns/[campaignId]/remind/route.js` — POST (admin): notify pending
  parents

Components:
- `components/feedback/FormBuilder.js` (client) — add text / single-choice /
  rating questions; **reorder with UP/DOWN BUTTONS (owner's locked choice —
  do NOT install @dnd-kit)**; delete; live preview beside it.
- `components/feedback/FormRenderer.js` (client) — renders a fields array for
  filling (textareas, radio groups, 1–5 stars). Shared by the parent tab/page
  and the builder's preview.

Pages (role-first URLs, as always):
- `app/admin/feedback/page.js` — templates (use previous / create new) +
  campaigns list (response counts, pending list + Send Reminder, Close,
  Report buttons)
- `app/parent/feedback/[campaignId]/page.js` — the fill screen
- The parent complaints page pattern (`/parent/complaints`) shows how to add a
  "pending forms" entry point; alternatively a card on `/parent` when a form
  is waiting.
- Optional: `app/admin/feedback/report/[campaignId]/page.js` — an on-screen
  version of the report before/alongside the PDF.

Home-page cards: add a FEEDBACK link card to `app/admin/page.js` and (when a
pending-form indicator exists) `app/parent/page.js` — one `.card` each, same
as the attendance/fees/complaints cards.

## 6. The report (pdfkit is now INSTALLED — reuse the working example)

- `app/api/fees/receipt/[receiptNumber]/route.js` is a WORKING pdfkit route in
  this repo — copy its structure (stream to Buffer, `Content-Type:
  application/pdf`, `Content-Disposition: attachment`).
- `next.config.mjs` already has `serverExternalPackages: ["pdfkit"]` — WITHOUT
  it pdfkit 500s looking for its font files. Never remove that line.
- Font gotcha: built-in Helvetica can't draw ₹ (write "Rs " in PDFs). Ratings
  and percentages are plain digits, so this hardly matters here.
- Aggregation rules (from the prompt, sane): per question, in template order —
  - `text`: every answer listed with the parent's name.
  - `rating`: the average ("Average: 4.2 / 5") + count.
  - `single_choice`: percentage per option ("Yes — 78%, No — 22%").
  - Footer: "Submitted: X/Y parents". Unanswered parents simply don't appear.

## 7. Notifications (open question — see below)

When built, use `lib/notify.js` (`createNotification`) only, source `'fees'`…
NO — source must be a schema-legal value; use `'system'`? NO — the right value
...
no dedicated value — **use `'complaints'`? No. Use `'system'` is the
...
1. **Notification source value** (above) — reuse `'complaints'` or approve the
   tiny migration adding `'feedback'` to `notifications_source_check`.
2. **Buzz levels:** my recommendation — campaign push = bell only; "Send
