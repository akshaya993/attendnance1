# Feature 01 — Attendance: Locked Decisions

Status: **DONE — built and tested 2026-08-13.**
...
## 1. THE WORKING-DAY RULE (the owner's rule — it overrides everything older)
...
attended    = workingDays − (absent × 1) − (half_day × 0.5)     [late = 1]
percentage  = attended / workingDays × 100, rounded to 1 decimal
```
...
cap exists — the rule above is the ONLY rule. Revisit only if a real school
...
|---|---|---|
| 1 | URLs | **Role-first:** `/teacher/attendance`, `/parent/attendance`, `/admin/attendance`, `/admin/attendance/class/[classId]`. Matches feature 09's pattern; proxy.js needed ZERO changes (its ROLE_PREFIXES already gate `/teacher`, `/parent`, `/admin`). The prompt file's `/attendance/teacher` layout was NOT used. |
...
| 8 | Multiple children | Minimal link-based picker on the parent page (`?student=<id>`), rendered only when the family has 2+ children. **Feature 11 replaces it** with the real ChildSwitcher — see `01-1-files-explained.md` for the handoff notes. |
...
- `lib/guard.js` — `requireActiveApiSession()` is the first line of every API
...
- `lib/auth.js` — `requireRole()` for role gates.
- `lib/audit.js` — `logAudit()` for overrides (standalone mode, same as
  feature 09's broadcast: a failed audit row never rolls back saved attendance).
- `lib/notify.js` — `createNotification()` is the ONLY thing writing
  notification rows. Attendance adds no delivery code of its own.
- `lib/db.js` — `query` / `withTransaction`.
- `coreRepo.listClassesByBranch()` — the teacher's class grid reuses it
...
- The bell, push subscriptions, service worker — untouched; absence alerts
  ride them through `lib/notify.js`.

### 3.2 Deliberately NOT created

- **`/api/attendance/classes` route** — the prompt file lists it, but the
...
- **A `workers/` job, a counter column, a new table** — nothing. The
...
### 4.1 `profile.branchId` is a STRING — use `session.branchId`
...
was `"4" !== 4` → always true → silent 404 on both parameterised pages.
...
bus `9000000021` — all `Pass@123`. NOTE: `9810000001`'s password no longer
matches (changed during some earlier feature's testing) — use 9810000002.

- Teacher submit class 49 with [417, 401] → `200 {absentCount:2, notified:2}`.
- Immediate second submit → `409` with a clean message; no partial rows.
- Teacher state read → `submitted:true, absentIds:[401,417], modifiedCount:0`.
- Teacher modify to [417] → `200 {absentCount:1, notified:0}` (no re-alert —
  417 was already absent; 401's removal is silent).
- Second teacher modify → `403 Modification limit reached - contact admin.`
- Modify on a never-submitted class → `404`.
- Parent summary for child 417: workingDays went 6 → **7** the moment the
  teacher submitted (the working-day rule working live), percentage
  recomputed correctly each time (83.3 → 71.4 → 85.7 as edits happened).
...
- Parent reading another family's child (ids 402, 418) → `403`.
- Admin dashboard: school % averages ONLY submitted classes; counter read
  `1/16` then `2/16` as classes submitted.
- Admin override class 49 → `200 {created:false, notified:1}` (only the newly
...
- Override on a class with NO submission creates it (`created:true` path used
  for class 50's empty-absent test: `absentCount: 0` — an all-present day
  still counts as a working day, exactly per the rule).
- Guards: parent POST submit → 403 · teacher GET admin-summary → 403 ·
  teacher POST override → 403 · bus POST submit → 403 · anonymous → 401 ·
  bad `classId=abc` → 400.
- Pages render: `/teacher/attendance` (grid + sheet), `/parent/attendance`
  (card "71.4%", history chips with "Thu, 13 Aug" on top), `/admin/attendance`,
  `/admin/attendance/class/49` — all HTTP 200 with expected content.
...
- **Weights on real data:** student 449 (1 late + 1 absent seeded) →
  attended = 7 − 1 − 0 = 6 → 85.7% (late costs nothing). Student 465 (1
  half_day + 1 absent) → 7 − 1 − 0.5 = 5.5 → 78.6% (half day costs exactly
...
- **All-present submissions count:** class 50's zero-absent submission raised
  its working days 6 → 7 with nobody marked absent.
- **Notification deltas, measured by unread counts:** empty submit →
  notified=0, bell unchanged (11→11). Modify adding one child → notified=1,
  bell 11→12, top item is the absence alert. Admin override [417,433] →
  notified=1: 433's parent 8→9, 417's parent stays 12 (no re-alert for a
...
1. **Feature 11 (Profiles):** replace the link-based child picker on
   `/parent/attendance` with the real `ChildSwitcher`. The page already loads
   `getChildrenOfParent()` and honours `?student=<id>` — the switcher only
   needs to drive that same param. Handoff details in `01-1-files-explained.md`.
2. **Staff tab for the admin page** — specced in `01-1-files-explained.md`;
...
3. **Feature 02 (Bus) / others:** to send attendance-style alerts from your own
   feature, call `createNotification()` with your own `source` — do NOT copy
